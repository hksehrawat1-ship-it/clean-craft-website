...
import { Calculator, IndianRupee, ArrowDown, CircleDollarSign, BadgeIndianRupee } from "lucide-react";
    ...
    <div className="bg-white rounded-xl shadow-lg overflow-hidden w-full max-w-md mx-auto google-shadow">
      <div className="p-6">
        <div className="flex items-center justify-center mb-6">
          <img 
            src="/lovable-uploads/d21f7dd7-7aba-47be-badd-9aa2ec3416f7.png" 
            alt="Clean Craft Laundry Franchise Logo" 
            className="h-16 w-16 object-contain"
          />
          <h3 className="font-semibold text-xl text-google-gray ml-2">Clean Craft</h3>
        </div>
        <h3 className="font-semibold text-xl text-cleancraft-darkgold mb-4 text-center">Laundry Franchise ROI Calculator</h3>
        ...
        <div className={`space-y-${isMobile ? '3' : '4'}`}>
          ...
          <div className="bg-cleancraft-light/10 p-3 rounded-lg">
            <div className="flex items-center mb-1">
              ...
              <label className="block text-sm font-medium text-gray-600">1. Initial Investment</label>
            </div>
          ...
          <div className="bg-cleancraft-light/10 p-3 rounded-lg">
            <div className="flex items-center mb-1">
              ...
              <label className="block text-sm font-medium text-gray-600">2. Expected Monthly Revenue</label>
            </div>
            <div className="relative">
              <span className="absolute left-3 top-2.5 text-gray-500">
                <IndianRupee className="h-4 w-4" />
          ...
          <div className="bg-cleancraft-light/10 p-3 rounded-lg">
            <div className="flex items-center mb-1">
              ...
              <label className="block text-sm font-medium text-gray-600">3. Profit Margin per month</label>
            </div>
            <div className={`relative ${showResults ? 'bg-cleancraft-light/50' : 'bg-cleancraft-light/20'} py-3 px-4 rounded-md transition-colors duration-300`}>
              <div className="flex items-center justify-between">
                <span className={`${showResults ? 'text-google-blue font-medium' : 'text-gray-400'} transition-colors duration-300`}>
                  {showResults ? `₹${profit.min} - ₹${profit.max}` : "Calculate to see results"}
                </span>
                <Badge variant="outline" className={`${showResults ? 'bg-google-green/20 text-google-green' : 'bg-gray-100 text-gray-400'} transition-colors duration-300`}>
                  {showResults ? '62-68% Margin' : 'Waiting'}
                </Badge>
              </div>
            </div>
            {showResults && (
              <p className="text-xs text-google-green mt-1">Industry standard profit margins in India!</p>
            )}
          </div>
