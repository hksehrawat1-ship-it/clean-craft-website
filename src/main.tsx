...
function renderWithErrorHandling() {
  try {
    ...
    const isMobileError =
      ...
      !error.message.includes("gptengineer"); // Ignore Lovable Script error
    ...
    if (isMobileError) {
      errorMessage =
        "This app may not be compatible with your browser version. Please try updating your browser or using a different one.";
...
window.addEventListener("unhandledrejection", (event) => {
  ...
  // Allow Strapi connection errors to continue without crashing the app
  if (
    event.reason?.message?.includes("strapi") ||
    event.reason?.message?.includes("fetch")
  ) {
    console.warn(
      "Strapi connection issue detected, but app should continue with fallback content"
    );
    return;
  }

  // Optional: You could add handling here for critical errors only if needed
});
